...
AVAILABLE_ESTIMATION_METHODS = ["logprob", "mean_logprob", "semantic_entropy", "ice", "self_consistency", "sar",  "inside", "mi", "p_true", "semantic_density"]
        ...
        if method in LOGPROB_BASED_METHODS:
            ...
            if mi_config["save_path"] is not None:
                ...
                if os.path.isdir(mi_config["save_path"]):
                    mi_save_path = os.path.join(mi_config["save_path"], "mi_sampling_results" + ".json")
                else:
                ...
                mi_save_path = os.path.join(mi_cache_dir, "mi_sampling_results" + ".json")
                logger.info(f"save sampling results to {mi_save_path}")
            
            if mi_config["save_mu2_path"] is not None:
                
                if os.path.isdir(mi_config["save_mu2_path"]):
                    mu2_save_path = os.path.join(mi_config["save_mu2_path"], "mi_mu2_results" + ".json")
                else:
                ...
                mu2_save_path = os.path.join(mi_cache_dir, "mi_mu2_results" + ".json")
                logger.info(f"save condititonal prob mu2 results to {mu2_save_path}")
